...
if(req.url==="/v1/chat/completions"&&req.method==="POST"){const payload=JSON.parse(await readBody(req)||"{}");const result=await runtime.router.execute(payload,async(model)=>{if(!model.adapter)throw Object.assign(new Error("ADAPTER_NOT_BOUND"),{code:"ADAPTER_NOT_BOUND"});return runtime.providers?.invoke(model,payload)??model.adapter.invoke(payload);});return json(res,200,result.result);}
